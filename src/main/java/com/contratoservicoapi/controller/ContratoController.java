...
import com.contratoservicoapi.service.ContratoService;
...
public class ContratoController {
    ...
    @GetMapping
    public List<ContratoResponseDTO> listar(@RequestParam(required = false) String objeto, @RequestParam(required = false) Long fornecedorId) {
        List<ContratoResponseDTO> resultado = service.listar();
        if (objeto != null && !objeto.isBlank()) {
            resultado = resultado.stream().filter(item -> item.getObjeto() != null &&
                item.getObjeto().toLowerCase().contains(objeto.toLowerCase()))
                .collect(java.util.stream.Collectors.toList());
        }
        if (fornecedorId != null) {
            resultado = resultado.stream().filter(item -> fornecedorId.equals(item.getFornecedorId())).collect(java.util.stream.Collectors.toList());
        }
        return resultado;
    }
